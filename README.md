...
The suite is offline by default. Set `YOUTUBE_API_KEY` — or `YOUTUBE_API_VIA_PROXY=true`
where a gateway attaches the key for you — and `npm test` additionally runs
...
an end-to-end stdio handshake against the built server, and — when a key is present, locally
or via the proxy — the live API checks above.
